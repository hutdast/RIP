...
//This is custom order addition the one with opencart is not set properly
  	 public function addRustikOrder($param) {
        $this->db->query("INSERT INTO `rustik_order` SET customer ='".$param['customer'] ."', "
                . "products = '".serialize($param['products'])."', status ='".$param['status']."', "
                . " date_added = NOW() , customer_id = ".$param['customer_id']);
         return $this->db->getLastId();
    }
//Delete an order in the event a payment is cancelled or void
    public function deleteRustikOrder($param) {
        $this->db->query("DELETE FROM `rustik_order` WHERE rustik_order_id = ".(int)$param);
    }



//save the customer invoice_id number 
    public function saveInvoice_num($param) {
        $query = $this->db->query("SELECT * FROM ". DB_PREFIX . "freshbook_invoice WHERE customer = '" . $this->db->escape($param['email']) . "'");
        if (count($query->rows) == 0) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "freshbook_invoice` SET invoice_id = '" . $this->db->escape($param['invoice_id'])
                    . "', customer = '" . $this->db->escape($param['email']) . "'");
            //else update it, because we do not want to have multiple invoice for one customer in one session
        } elseif (count($query->rows) == 1) {
            $this->db->query("UPDATE `" . DB_PREFIX . "freshbook_invoice` SET invoice_id = '" . $this->db->escape($param['invoice_id'])
                   . "' WHERE customer = '" . $this->db->escape($param['email']) . "'");
        }
    }

    //get customer invoice number
    public function getInvoice_num($param) {
  $query = $this->db->query("SELECT invoice_id FROM " . DB_PREFIX . "freshbook_invoice WHERE customer = '" . $this->db->escape($param) . "' LIMIT 1");
  if($query->row){
...
//RIP modifications: adding functionality that acceses freshbook api
    ...
    public function addOrder($data) {
        ...
        if (isset($data['products'])) {
            foreach ($data['products'] as $product) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int) $order_id . "', product_id = '"
                        . (int) $product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '', quantity = '"
                        . (int) $product['quantity'] . "', price = '"
                        . (float) $product['price'] . "', total = '" . (float) $product['total'] . "', tax = '', reward = ''");
