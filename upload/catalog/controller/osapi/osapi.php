...
/*
    ...
    OneSaas Connect API 2.0.6.38 for OpenCart v1.5.4.1
...
class ControllerOsapiOsapi extends Controller {
  ...
  // Functions
    ...
    public function index() {
        ...
        */
        $xml->addAttribute('Version','2.0.6.38');
                ...
                case "Orders":
                    ...
                    foreach ($orders_id as $order_id) {
						...
						foreach ($order_totals as $order_total) {
							...
							{
								$hasDiscount = true;
								$coupon_code = $this->getStringBetween($order_total['title'],'(',')');
								$coupon_code_query = $this->db->query("SELECT DISTINCT * FROM ". DB_PREFIX ."coupon WHERE code = '" . $this->db->escape($coupon_code) . "'")->row;
								if($coupon_code_query['type'] == 'P')
									{	
										$hasFixedDiscount = false;
										$discount_value = $coupon_code_query['discount']/100;
									}
								if($coupon_code_query['type'] == 'F')
									{
										$hasFixedDiscount = true;
										$discount_value = $coupon_code_query['discount'];										
									}								
							}
                        ...
                        foreach($item_results as $item_result) {
											...
											if($tax_result['code'] == 'tax')
												{
													...
													$itemTax = $itemTaxes->addChild('Tax');
														...
														if (isset($item_result['quantity']) && isset($tax_rate['rate'])) {
															$itemTax->TaxAmount = ($item_result['price']*$item_result['quantity'])*$tax_rate['rate']/100;
															$item_discount_tax_value += ($discount_value/$item_result['quantity'])*$tax_rate['rate']/100;
														}   
												}																						
										}		
								}
							if($hasFixedDiscount && $discount_value!= 0)
							{
								$line_discount = $discount_value/$item_result['quantity'];
								$line_price = $item_result['price']; 
								$total = $subTotal/$item_result['quantity'];
								if($line_discount >= $line_price)
								{
									if($line_discount == $line_price || $line_discount >= $total)
									{
										$discount_value = 0;
										$item->Discount = $item_result['price']+$item_result['tax'];	
									}
									else
									{
										$discount_value = $discount_value - ($line_discount*$item_result['quantity']);
										$item->Discount = $line_discount + $item_discount_tax_value;
									}									
								}		
								else
								{	
							...
							{
							    $item->Discount = ($item_result['price']+$item_result['tax'])*$discount_value;									
							}
						...
						foreach ($order_totals as $order_total) {
                            if ($order_total['code']=='shipping') {
								...
								$shipping_amount = $order_total['value']; 
									if( $tax_sort_order > 0 && $tax_sort_order > $order_total['sort_order'] )	
										{	
										$shipping_tax_amount = 0;
										$shipping_taxes = $shipping->addChild('Taxes');											
											foreach ($order_totals as $order_total) {
													...
													{
													$tax_name = htmlspecialchars($order_total['title']); 
													$tax_rate = $this->db->query("SELECT * from " . DB_PREFIX . "tax_rate WHERE name = '".$order_total['title']."'")->row;
													$shipping_tax = $shipping_taxes->addChild('Tax');
														if (isset($order_total['title'])) {
															$shipping_tax->TaxName = $tax_name;																
														}													
														if (isset($tax_rate['rate'])) {
															$shipping_tax->TaxRate = 0.0 + $tax_rate['rate']/100;
														}											
														if (isset($tax_rate['rate'])) {
															$shipping_tax_amount += $shipping_amount*$tax_rate['rate']/100;
															$shipping_tax->TaxAmount = $shipping_amount*$tax_rate['rate']/100;
														}													
													}
											}											
										}
								$shipping->Amount = $shipping_amount + $shipping_tax_amount;					
							}
						}	
                        ...
                        foreach ($order_totals as $order_total) {
									...
									if ($order_total['code']=='tax')
										{
											...
											$tax_rate = $this->db->query("SELECT * from " . DB_PREFIX . "tax_rate WHERE name = '".$order_total['title']."'")->row;
												if (isset($tax_rate['rate'])) {
													$discount_tax_amount += $discounts*$tax_rate['rate']/100;
												}													
										}
									}
								}
							...
							if (array_key_exists('code', $order_total) && $order_total['code']!='total' && $order_total['code']!= 'sub_total' && $order_total['code']!='tax'&& $order_total['code']!='shipping' && $order_total['code']!='coupon' && $order_total['code']!='voucher' && $order_total['code']!='POS') {
									...
									else
									{
										...
										if(isset($tax_sort_order))
										{										
											if($tax_sort_order > 0 && $tax_sort_order > $order_total['sort_order'])
											{
												$otherCharge_tax_amount = 0;
												$tax_results = $this->model_account_order->getOrderTotals($order['order_id']);
												$chargeTaxes = $charge->addChild('Taxes');
												foreach($tax_results as $tax_result)
												{
													if($tax_result['code'] == 'tax')
														{
															$tax_name = htmlspecialchars($tax_result['title']); 
															$tax_rate = $this->db->query("SELECT * from " . DB_PREFIX . "tax_rate WHERE name = '".$tax_result['title']."'")->row;	
															$chargeTax = $chargeTaxes->addChild('Tax');
																if (isset($tax_result['title'])) {
																	$chargeTax->TaxName = $tax_name;																
																}													
																if (isset($tax_rate['rate'])) {
																	$chargeTax->TaxRate = 0.0 + $tax_rate['rate']/100;
																}											
																if (isset($item_result['quantity']) && isset($tax_rate['rate'])) {
																	$chargeTax->TaxAmount = $order_total['value']*$tax_rate['rate']/100;
																	$otherCharge_tax_amount += $order_total['value']*$tax_rate['rate']/100;
																}   
														}																						
												}											
											}
										}										
										$charge->Amount = $order_total['value']+ $otherCharge_tax_amount + 0.0;
									}
								}
						}								
                        $xml_order->Discounts = $discounts + $discount_tax_amount;
/*						
                            ...
                            if ($order_total['code']=='shipping') {
								...
								if (array_key_exists('code', $order_total) && $order_total['code']!='total' && $order_total['code']!= 'sub_total' && $order_total['code']!='tax'&& $order_total['code']!='coupon'&& $order_total['code']!='voucher' && $order_total['code']!='POS') {
									...
									else
									{
										...
										if(isset($tax_sort_order))
										{										
											if($order_total['sort_order'] < $tax_sort_order )
											{
												$tax_results = $this->model_account_order->getOrderTotals($order['order_id']);
												$chargeTaxes = $charge->addChild('Taxes');
												foreach($tax_results as $tax_result)
