...
class ControllerCustomerCustomer extends Controller {
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////       
            ////////////////////////////////////////////////////////////////////////////////////////////////
            //RIP modification: adding customer to freshbooks

            $client_info = array($this->request->post['firstname'], $this->request->post['lastname'], $this->request->post['folder_name'], $this->request->post['email'],
                $this->request->post['email'], 'password', $this->request->post['email'], $this->request->post['telephone'], '', '', '', '', '', '');



            $poststring = '';
            $client_request = explode("~!", $this->db->freshbooks('client')['request']);
            $client_info_index = 0;
...
$api = array();
            ...
            if(count($this->model_setting_setting->getFresbooks('rustik')) > 0 ){
               $api =  $this->model_setting_setting->getFresbooks('rustik');
	...
	//If services is deactivated then we skip over this part
		if(strpos($result, '<?xml') != false){
           $response = json_decode(json_encode(simplexml_load_string($result)), true);

            //If response is ok there is a connection else we have to let administrator know
        $this->error['freshbooks'] ='';
echo $response['@attributes']['status'];

	if($response['@attributes']['status'] == 'ok'){
		 $this->error['freshbooks'] ='';
	}else{
                $this->error['freshbooks'] = 'Unable to establish communication with freshbooks';
            }
	}//End of strpos check

     }//end of APi existence check
    ...
    protected function getList() {
        ...
        //Set the error for apis connection
        if (isset($this->error['freshbooks'])) {
            $data['error_warning'] = $this->error['freshbooks'];
        } else {
            $data['error_warning'] = '';
        }
    ...
    protected function getForm() {
        ...
        ///////////////////////////////////// /////////////////////////////////////
        ///////////////////////////////////// RIP modifications: making sure the folder is being created/////////////////////////////////////
        if (isset($this->error['folder_name'])) {
            $data['error_folder_name'] = $this->error['folder_name'];
        } else {
            $data['error_folder_name'] = '';
        }
    ...
    protected function validateForm() {
        ...
        //RIP modifications:
        //if Folder is not created, it is imperative to stop the validation of the form
        //Prep the customers array to get all the folders
        $param = array(
            'folder_name' => $this->request->post['folder_name'],
           'email' => $this->request->post['email']);
        
        if (!in_array($this->request->post['folder_name'], scandir(DIR_IMAGE . "catalog"))) {
            $this->error['folder_name'] = ' Folder is not created';
            //Else check to see if another user already has that folder name
        }
