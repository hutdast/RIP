...
class ControllerModuleOsapi extends Controller {
	...
	public function index() {
		...
		//$ak_query = $this->db->query("select s.key from " . DB_PREFIX . "setting s where s.key = 'OSAPI_ACCESS_KEY' and s.group='OSAPI'");
                $ak_query = $this->db->query("select s.key from " . DB_PREFIX . "setting s where s.key = 'OSAPI_ACCESS_KEY'");
		if ($ak_query->num_rows == 0) {
			...
			//$this->db->query("INSERT INTO " . DB_PREFIX . "setting VALUES (NULL, 0, 'OSAPI', 'OSAPI_ACCESS_KEY', CONCAT(MD5(NOW()), MD5(CURTIME())), 0)");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "setting VALUES (NULL, 0, 'OSAPI', 'OSAPI_ACCESS_KEY', CONCAT(MD5(NOW()), MD5(CURTIME())) )");
			// Inizialise Version
		...
		// Check Version
		$version_query = $this->db->query("select s.value from " . DB_PREFIX . "setting s where s.key = 'OSAPI_VERSION' ");
			if ($version_query->num_rows == 0) {
				...
				if ($version != $this->os_version) {
					$this->db->query("UPDATE " . DB_PREFIX . "setting s SET s.value='" . $this->os_version . "' where s.key = 'OSAPI_VERSION'");
				}
		}
		// Read AccessKey
		$ak = '';
		$ak_query = $this->db->query("select s.value from " . DB_PREFIX . "setting s where s.key = 'OSAPI_ACCESS_KEY' ");
		if ($ak_query->num_rows == 1) {
			$ak = $ak_query->row['value'];
                       
		}
		// Read OneSaas Url
		$os_url_query = $this->db->query("select s.value from " . DB_PREFIX . "setting s where s.key = 'OSAPI_IFRAME_URL'");
