...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		// Get files
		$files = glob($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,JPG,JPEG,PNG,GIF,bmp}', GLOB_BRACE);
 ...
 //RIP modifications: upload multiple files
				...
				// If path is a directory beging deleting each file and sub folder
					...
					foreach ($files as $file) {
						...
						// If directory use the remove directory function
							  ...
							  //Go inside the directory and delete evrything
                            $allfiles = scandir($file);
                            foreach ($allfiles as $a_file) {
                                if (substr($a_file, 0, 1) != '.' ) {
                                    unlink($file."/".$a_file);
                                }
                            }
                            rmdir($file);
